...
contract StratX is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public newestEurn;

    address public constant usdtAddress =
        0xa71EdC38d189767582C38A3145b5873052c3e47a;

    address public farmfinanceAddress;
    ...
    uint256 public controllerFee = 88;
    ...
    constructor(
        ...
        farmfinanceAddress = _farmfinanceAddress;
        ...
        if (isAutoComp) {
            ...
            earnedToAUTOPath = [earnedAddress, usdtAddress, burnAddress];
            if (usdtAddress == earnedAddress) {
                earnedToAUTOPath = [usdtAddress, burnAddress];
            }

            earnedToToken0Path = [earnedAddress, usdtAddress, token0Address];
            if (usdtAddress == token0Address) {
                earnedToToken0Path = [earnedAddress, usdtAddress];
            }

            earnedToToken1Path = [earnedAddress, usdtAddress, token1Address];
            if (usdtAddress == token1Address) {
                earnedToToken1Path = [earnedAddress, usdtAddress];
            }

            token0ToEarnedPath = [token0Address, usdtAddress, earnedAddress];
            if (usdtAddress == token0Address) {
                token0ToEarnedPath = [usdtAddress, earnedAddress];
            }

            token1ToEarnedPath = [token1Address, usdtAddress, earnedAddress];
            if (usdtAddress == token1Address) {
                token1ToEarnedPath = [usdtAddress, earnedAddress];
            }
        }

        transferOwnership(farmfinanceAddress);
    }
        ...
        returns (uint256)
    {
        ...
        IERC20(wantAddress).safeTransfer(farmfinanceAddress, _wantAmt);
    ...
    function earn() public whenNotPaused {
        ...
        lastEarnBlock = block.number;
        newestEurn = IERC20(wantAddress).balanceOf(address(this));
        _farm();
    }
